...
total_us_cases <- national %>% filter(date == max(date, na.rm = TRUE)) %>% summarize(total_us_cases = cases) %>% pull(total_us_cases)
...
total_us_deaths <- national %>% filter(date == max(date, na.rm = TRUE)) %>% summarize(total_us_deaths = deaths) %>% pull(total_us_deaths)


# 2.c Which state has had the highest number of COVID cases? Make sure to pull() this value `state_highest_cases`
state_highest_cases <- states %>% group_by(state) %>% summarize(cases = sum(cases, na.rm = TRUE)) %>% filter(cases == max(cases, na.rm = TRUE)) %>% pull(state)

# 2.d What is the highest number of cases in a state? Make sure to pull() this number `num_highest_state`
num_highest_state <- states %>% group_by(state) %>% summarize(cases = sum(cases, na.rm = TRUE)) %>% filter(cases == max(cases, na.rm = TRUE)) %>% pull(cases)
...
state_highest_ratio <- states %>% group_by(state) %>% summarize(cases_sum = sum(cases), deaths_sum = sum(deaths)) %>% mutate(ratio = deaths_sum/cases_sum) %>% filter(ratio == max(ratio, na.rm = TRUE)) %>% pull(state)
...
state_lowest_cases <- states %>% group_by(state) %>% filter(date == max(date, na.rm = TRUE)) %>% summarize(cases = sum(cases, na.rm = TRUE)) %>% filter(cases == min(cases, na.rm = TRUE)) %>% pull(state)
...
county_highest_cases <- counties %>% group_by(county) %>% summarize(cases = max(cases, na.rm = TRUE)) %>% filter(cases == max(cases, na.rm = TRUE)) %>% pull(county)
...
num_highest_cases_county <- counties %>% group_by(county) %>% summarize(cases = max(cases, na.rm = TRUE)) %>% filter(cases == max(cases, na.rm = TRUE)) %>% pull(cases)
...
location_most_deaths <- counties %>% group_by(location) %>% summarize(cases = sum(cases, na.rm = TRUE)) %>% filter(cases == max(cases, na.rm = TRUE)) %>% pull(location)
...
date_most_cases <- national %>% group_by(date) %>% summarize(new_cases = sum(new_cases, na.rm = TRUE)) %>% filter(new_cases == max(new_cases, na.rm = TRUE)) %>% pull(date)

# 2.n What was the date when the most new deaths in the U.S. occurred? Make sure to pull() this value `date_most_deaths`
date_most_deaths <- national %>% group_by(date) %>% summarize(new_deaths = sum(new_deaths, na.rm = TRUE)) %>% filter(new_deaths == max(new_deaths, na.rm = TRUE)) %>% pull(date)


# 2.o How many people died on the date when the most deaths occurred? Make sure to pull() this value `most_deaths`
most_deaths <- date_most_deaths <- national %>% group_by(date) %>% summarize(new_deaths = sum(new_deaths, na.rm = TRUE)) %>% filter(new_deaths == max(new_deaths, na.rm = TRUE)) %>% pull(new_deaths)
...
highest_cases_in_each_state <- counties %>% group_by(state, county) %>% summarize(cases = max(cases, na.rm = TRUE)) %>% filter(cases == max(cases, na.rm = TRUE))
...
lowest_deaths_in_each_state <- counties %>% group_by(state, county) %>% summarize(deaths = max(deaths, na.rm = TRUE)) %>% filter(deaths == min(deaths, na.rm = TRUE))
...
national_county_diff <- all_totals %>% group_by(cases) %>% filter(county_total_cases != cases)
...
# What state has more deaths, virginia or north carolina
#

#  Write code (at least 2-3 lines) that will answer your question
my_answer <- states %>% group_by(state) %>% summarize(deaths = sum(deaths, na.rm = TRUE))
virginia_deaths = my_answer %>% filter(state == "Virginia") %>% summarize(deaths) %>% pull(deaths)
North_Carolina_Deaths = my_answer %>% filter(state == "North Carolina") %>% summarize(deaths) %>% pull(deaths)
if(virginia_deaths > North_Carolina_Deaths){
  print("Virginia has more deaths")
}else{
  print("North Carolina has more deaths")
}
