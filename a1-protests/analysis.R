...
# 2017-01-16, Johnson City, TN, 300, Civil Rights; For racial justice; Martin Luther King, Jr., Yes, http://www.johnsoncitypress.com/Local/2017/01/16/H, 4
...
x <- str_detect(locations, "WA")
num_in_wa <- sum(x, na.rm = TRUE)
...
count_protests_in_location <- function(location) {
  y <- str_detect(locations,location)
num_in_location <- sum(y, na.rm = TRUE)
final <- paste("There were", num_in_location, "protests in", location, ".")
...
states <- c(str_sub(locations, start = -2))
# (3.i) How many unique states are in the dataset? 
# 61 unique states
#Create a vector of just the unique states in the dataset Save the unique states in a variable called `uniq_states` (1 point)
...
num_purposes <- length(unique(purposes, na.rm = TRUE))
...
table <- table(high_level_purposes)
View(table)
...
# Congrats! You're finished. Don't forget to save, push all changes to GitHub, and submit the link to your repository on Canvas!iew()